...
\msb{A weak differentiating sentence: } But the connections between
...
Crowd--power has continued to parallel historical piecework's trajectory
by outgrowing ad--hoc worker groups and coalescing into more formal working groups \msb{give an example, since the factorization claim is really gutsy right now otherwise};
...
Making sense of this \msb{of what?} 
...
This paper will attempt to demonstrate that the familiarity \msb{what familiarity?} is more than passing;
...
in the study of crowd and gig work. \msb{nice paragraph}
...
\cite{crowdworkFuture}. \msb{split up that sentence. One is the def'n of piecework, the other is the comparison you're drawing}
...
% \cite{scott1975women}.
Given the scope,
we can frame piecework and gig work \msb{wait, are you using gig work as the umbrella term? I think you need a category term that covers all of them} as sharing these important similarities:
\begin{inlinelist}
...
or ``gig work'' \msb{so is ``gig work'' your umbrella term? we need a definition of this at some point}, by evaluating this work through a more familiar lens: piecework.
...
The existing body of research has shed light on on--demand \msb{missing noun?} from various perspectives,
...
\msb{the past two paragraphs feel to me like they should be riddled with citations to the historical literature. They feel kind of bald without them.}
...
\cite{bernsteinSoylent}. \msb{given future focus on Foundry, this is too bernstein-centric. instead, talk about crowdforge's map-reduce framework and Little's Human Computation Algorithms HCOMP 2009 with its iterative algorithms}
...
of crowd work that microtasks should be designed flexibly \msb{not sure what flexibly means here, and the rest of the sentence isn't helping me unpack it. flexibility for requester? worker? flexible algorthms?},
