...
\ali{I'm interleaving the paragraphs, so this paragraph needs to be refactored a bit to reflect that instead of suggesting that the road map will look like two distinct threads.
I'll probably try to take a shot at it in a bit, but for now don't take the wording too seriously.}
...
\ali{
So I'm thinking that this should flow like this:
``We'll try to understand what piecework was by tracing the development of and discourse around piecework.''
This'll give us the ability to switch back and forth between the timeline of the practice of piecework
--- that is, the ``piecework primer'' stuff down below ---
and the contemporary/concurrent research going on around the same time
(i.e. this stuff immediately following).

I'm going to interleave the paragraphs, but
if you think that's a flawed approach, then I'll bet you won't like what you'll see below that much.%
}
...
\ali{This is worded a little awkwardly;
I want to say that we're looking at
the discourse about piecework at the same time that we look at
how piecework is unfolding. (wait, is that how I should word it?)}
...
to other countries
\item the quality of ``multidimensional'' work becoming too difficult to evaluate~\cite{hart2013rise}.
\end{inlinelist}}


\ali{This feels like the Australia of paragraphs: one sentence constitutes the whole paragraph.
Should I add some analysis here?}
...
\ali{there's no need for this paragraph:
